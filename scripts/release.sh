#!/usr/bin/env bash
#
# Release automation for app-deploy. Covers both distribution channels:
#   - Homebrew (infinum/tap)  - primary, macOS developer machines + macOS CI
#   - npm (@infinum/app-deploy) - secondary, Linux CI without Homebrew
#
# Usage:
#   scripts/release.sh prepare <version> [--direct] [--dry-run]
#   scripts/release.sh publish           [--direct] [--dry-run]
#
# prepare  Writes <version> into app-deploy.sh and package.json, commits, pushes
#          and opens a PR (or pushes straight to master with --direct).
#          Run on master to get a release/v<version> branch, or on an existing
#          feature branch to fold the bump into that branch's PR.
#
# publish  Run on an up-to-date master once the prepare PR is merged. Tags,
...
REPO_SLUG="infinum/app-deploy-script"
MAIN_BRANCH="master"
NPM_PACKAGE="@infinum/app-deploy"
TAP_SLUG="infinum/homebrew-tap"
TAP_MAIN_BRANCH="main"
FORMULA_FILE="Formula/app-deploy.rb"
...
current_version() {
    sed -n 's/^VERSION="\([^"]*\)"$/\1/p' app-deploy.sh
}
...
tap_dir() {
    local dir
    dir="${APP_DEPLOY_TAP_DIR:-$(brew --repository "$TAP_SLUG" 2> /dev/null || true)}"
    [ -n "$dir" ] && [ -d "$dir/.git" ] || fail "Homebrew tap clone not found. Run 'brew tap infinum/tap' or set APP_DEPLOY_TAP_DIR."
    echo "$dir"
}
...
prepare() {
    ...
    run sed -i '' "s/^VERSION=\"$current\"$/VERSION=\"$version\"/" app-deploy.sh
    run npm version "$version" --no-git-tag-version --allow-same-version
    if [ "$DRY_RUN" != "true" ]; then
        [ "$(current_version)" == "$version" ] || fail "Failed to update VERSION in app-deploy.sh."
        [ "$(package_version)" == "$version" ] || fail "Failed to update version in package.json."
    fi
    ok "app-deploy.sh and package.json now at $version"

    info "Committing and pushing"
    run git add app-deploy.sh package.json
...
publish() {
    ...
    [ -n "$version" ] || fail "Could not read VERSION from app-deploy.sh."
    pkg_version="$(package_version)"
    [ "$pkg_version" == "$version" ] || fail "Version mismatch: app-deploy.sh has $version, package.json has $pkg_version. Run 'prepare' first."
    tag="v$version"
    ...
    tap_branch="app-deploy-$version"
    tap_pr_title="app-deploy $version"
    ...
    run git -C "$worktree" commit -q -m "app-deploy $version"
    if [ "$DIRECT" == "true" ]; then
    ...
    echo
    ok "Release $tag done."
    echo "  Homebrew users: brew update && brew upgrade app-deploy   (after the tap PR is merged)"
